...
class ParseServicer(parse_pb2_grpc.GrpcServiceServicer):
    def ParseFont(self, requset,context):
        url = requset.url
        data = requset.data
        ...
        #     f.write(content)
        # fonts = TTFont(url)
        # unicodes = fonts.getGlyphOrder()[2:]
        # base_dict = {}
        
        # fonts = {"address":TTFont("7cc1dac6.woff")}
        return parse_pb2.ResponseData(data="", code=1)
    

def server():
    server = grpc.server(futures.ThreadPoolExecutor(max_workers=10))
    parse_pb2_grpc.add_GrpcServiceServicer_to_server(ParseServicer(), server)
